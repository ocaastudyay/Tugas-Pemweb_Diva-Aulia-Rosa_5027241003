...
const EditProduk = () => {
    ...
    useEffect(() => {
        const fetchProdukData = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:5000/api/produk/${id}`, { headers: { 'x-auth-token': token } });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/produk/${id}`, formData, { headers: { 'x-auth-token': token } });
            setMessage('Produk berhasil diperbarui!');
            
            setTimeout(() => { navigate('/'); }, 2000);

        } catch (error) {
            const errorMsg = error.response?.data?.msg || "Gagal memperbarui produk. Periksa input.";
            setMessage(errorMsg);
        }
    };
    ...
    const buttonStyle = { width: '100%', padding: '12px', backgroundColor: 'var(--color-accent-edit)', color: 'var(--color-text-dark)', border: 'none', borderRadius: '5px', cursor: 'pointer', marginTop: '20px' };

    if (loading) return <div style={{padding: '20px'}}>Memuat data produk...</div>;

    return (
        <div style={formStyle}>
            <h2 style={{ borderBottom: '1px solid var(--color-primary)', paddingBottom: '10px' }}>Edit Produk: {formData.nama}</h2>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" style={buttonStyle} onMouseOver={(e) => e.target.style.backgroundColor = 'var(--color-secondary)'} onMouseOut={(e) => e.target.style.backgroundColor = 'var(--color-accent-edit)'}>
                    Simpan Perubahan
                </button>
            </form>
        </div>
